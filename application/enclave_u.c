...
static sgx_status_t SGX_CDECL enclave_print_ocall(void* pms)
{
...
static sgx_status_t SGX_CDECL enclave_fwrite_enclave_memory_ocall(void* pms)
{
	ms_fwrite_enclave_memory_ocall_t* ms = SGX_CAST(ms_fwrite_enclave_memory_ocall_t*, pms);
	ms->ms_retval = fwrite_enclave_memory_ocall((const void*)ms->ms_buffer, ms->ms_size, ms->ms_count, ms->ms_stream);
	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL enclave_fread_copy_into_enclave_memory_ocall(void* pms)
{
	ms_fread_copy_into_enclave_memory_ocall_t* ms = SGX_CAST(ms_fread_copy_into_enclave_memory_ocall_t*, pms);
	ms->ms_retval = fread_copy_into_enclave_memory_ocall(ms->ms_buffer, ms->ms_size, ms->ms_count, ms->ms_stream);
	return SGX_SUCCESS;
}
...
static const struct {
	...
	8,
	{
		(void*)(uintptr_t)enclave_print_ocall,
		(void*)(uintptr_t)enclave_rewind_ocall,
		(void*)(uintptr_t)enclave_fseek_ocall,
		(void*)(uintptr_t)enclave_ftell_ocall,
		(void*)(uintptr_t)enclave_fwrite_enclave_memory_ocall,
		(void*)(uintptr_t)enclave_fread_copy_into_enclave_memory_ocall,
